...
+---------------------------+                   +--------------------------------------+
| HeartbeatThread           | PING_REGISTERED   | ManagementModule                     |
|  (instance_id, 10s)  -----+------------------>|  ping_registered(id, expected)       |
|  starts after REGISTER,   |   (NORMAL pool)   |  reaper thread (scan = timeout/4)    |
|   (starts healthy)        |                   |   -> reap_stale_instances(           |
|  outage or missing ctx    |  missing types    |        timeout, registration_grace)  |
|   -> re-register callback |<------------------+   -> drop_instance_state(id) fan-out |
|                           |   REGISTER        |                                      |
|                           +------------------>|        |                |            |
...
server availability and refreshes every matching Context; `None` marks an
untracked prober such as the scheduler adapter. PING keeps its BLOCKING
...
worker *should* enter degraded mode (the same back-pressure signal).
