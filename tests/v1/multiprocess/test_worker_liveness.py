...
def test_registration_aware_ping_returns_exact_missing_contexts() -> None:
    gpu = _FakeTarget()
    qstore = _FakeTarget()
    ...
    missing = mgmt.ping_registered(
        42,
        [RequestType.REGISTER_KV_CACHE, RequestType.REGISTER_Q_CACHE],
    )

    assert missing == [RequestType.REGISTER_Q_CACHE]
    assert gpu.touched == [42]
    assert qstore.touched == [42]
    # Compatibility PING still reports server availability even when the
    # instance is absent from every context.
