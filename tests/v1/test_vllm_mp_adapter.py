...
def test_missing_registration_recovers_even_when_heartbeat_was_healthy(
    monkeypatch,
) -> None:
    """A reachable server reporting a missing Context forces recovery even
    without an unhealthy-to-healthy edge."""
...
def test_unreachable_registered_ping_does_not_blindly_reregister(monkeypatch) -> None:
    monkeypatch.setattr(
        adapter_mod,
        "send_registered_ping",
        lambda req_client, timeout, instance_id, expected_registration_types: None,
    )
    health_event = threading.Event()
    health_event.set()
    heartbeat = HeartbeatThread(
...
def test_complete_registered_ping_does_not_reregister_healthy_worker(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        adapter_mod,
        "send_registered_ping",
        lambda req_client, timeout, instance_id, expected_registration_types: [],
    )
    health_event = threading.Event()
    health_event.set()
    heartbeat = HeartbeatThread(
